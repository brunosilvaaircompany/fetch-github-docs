...
to specify a fixed version of the pack. The `extractor` defines which language the CLI will use to create test databases from the code files stored in this CodeQL pack. For more information, see [Customize Analysis](https://docs.github.com/en/code-security/tutorials/customize-code-scanning/customize-analysis).
...
For full details of all the options you can use when testing queries, see [Test Run](https://docs.github.com/en/code-security/reference/code-scanning/codeql/codeql-cli-manual/test-run).
...
1. If you haven’t already added your custom queries to a CodeQL pack, create a CodeQL pack now. For example, if your custom Java queries are stored in `custom-queries/java/queries`, add a `qlpack.yml` file with the following contents to `custom-queries/java/queries`:
   ...
   For more information about CodeQL packs, see [Customize Analysis](https://docs.github.com/en/code-security/tutorials/customize-code-scanning/customize-analysis).

1. Create a CodeQL pack for your Java tests by adding a `qlpack.yml` file with the following contents to `custom-queries/java/tests`, updating the `dependencies` to match the name of your CodeQL pack of custom queries:
...
1. Extracts a CodeQL database from the `.java` files stored in the `EmptyThen` directory.
...
1. Compiles the query referenced by the `EmptyThen.qlref` file.
   ...
   For information about saving the search path as part of your configuration, see [Specify Command Options](https://docs.github.com/en/code-security/how-tos/find-and-fix-code-vulnerabilities/scan-from-the-command-line/specify-command-options).
...
`Test.java` example code. For more information, see [Manage Codeql Databases](https://docs.github.com/en/code-security/how-tos/find-and-fix-code-vulnerabilities/scan-from-vs-code/manage-codeql-databases#choosing-a-database-to-analyze).

## Further reading

* [CodeQL queries](https://codeql.github.com/docs/writing-codeql-queries/codeql-queries/#codeql-queries)
* [Testing Codeql Queries In Vs Code](https://docs.github.com/en/code-security/how-tos/find-and-fix-code-vulnerabilities/scan-from-vs-code/testing-codeql-queries-in-vs-code).
