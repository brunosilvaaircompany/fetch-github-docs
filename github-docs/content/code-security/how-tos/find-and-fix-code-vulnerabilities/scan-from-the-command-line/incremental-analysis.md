...
* **CodeQL CLI bundle version:** 2.21.0 or later for diff-informed analysis; 2.23.8 or later for overlay analysis (with per-language minimums, see [Minimum CLI bundle versions](#minimum-cli-bundle-versions))
...
* `--additional-packs` tells CodeQL where to find the pack on disk. For more information, see [Database Run Queries](https://docs.github.com/en/code-security/reference/code-scanning/codeql/codeql-cli-manual/database-run-queries).
* `--extension-packs` tells CodeQL to load the named extension pack.
...
For more information, see [Create Query Suites](https://docs.github.com/en/code-security/tutorials/customize-code-scanning/create-query-suites#filtering-the-queries-in-a-query-suite).
