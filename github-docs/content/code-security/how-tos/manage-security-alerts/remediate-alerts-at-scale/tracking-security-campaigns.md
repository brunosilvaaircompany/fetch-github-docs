...
To display the campaign tracking view, navigate to the **{% octicon "shield" aria-hidden="true" aria-label="shield" %} Security and quality** tab for the organization, then in the left sidebar click **{% octicon "goal" aria-hidden="true" aria-label="goal" %} Campaigns**. To display campaigns for secrets, click the **Secrets** tab at the top of the page.
