...
1. The AutoScalingRunnerSet Controller calls the API one more time to either fetch or create a runner scale set in the GitHub Actions service before creating the Runner ScaleSet Listener resource.
...
1. When a workflow run is triggered from a repository, the GitHub Actions Service dispatches individual job runs to the runners or runner scale sets where the `runs-on` key matches the name of a runner scale set or the labels of a runner scale set or self-hosted runner.
...
Because custom resources are extensions of the Kubernetes API, they won't be available in a default Kubernetes installation. You will need to install these custom resources to use ARC. For more information on installing custom resources, see [Get Started](https://docs.github.com/en/actions/tutorials/use-actions-runner-controller/get-started).

Once the custom resources are installed, you can deploy ARC into your Kubernetes cluster. For information about deploying ARC, see [Deploy Runner Scale Sets](https://docs.github.com/en/actions/how-tos/manage-runners/use-actions-runner-controller/deploy-runner-scale-sets).
...
* Use a base image that can run the self-hosted runner application. See [Self Hosted Runners](https://docs.github.com/en/actions/how-tos/manage-runners/self-hosted-runners).
...
If you're new to ARC, see [Get Started](https://docs.github.com/en/actions/tutorials/use-actions-runner-controller/get-started) to try out the basics.

When you're ready to use ARC to execute workflows, see [Use Arc In A Workflow](https://docs.github.com/en/actions/how-tos/manage-runners/use-actions-runner-controller/use-arc-in-a-workflow).
...
You can use the installation name of the runner scale set, or define the value of the `runnerScaleSetName` field in your [`values.yaml`](https://github.com/actions/actions-runner-controller/blob/master/charts/gha-runner-scale-set/values.yaml) file, as your `runs-on` target. You can also assign multiple labels to a scale set to enable more flexible job routing. To configure labels for a runner scale set, set the `runnerScaleSetLabels` value in your `values.yaml` file.
 See [Use In A Workflow](https://docs.github.com/en/actions/how-tos/manage-runners/self-hosted-runners/use-in-a-workflow).

You can scale runners statically or dynamically depending on your needs. See [Deploy Runner Scale Sets](https://docs.github.com/en/actions/how-tos/manage-runners/use-actions-runner-controller/deploy-runner-scale-sets).
