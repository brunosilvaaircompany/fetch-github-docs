...
* **Inherited CLI and SDK safety controls**: App sessions use GitHub Copilot CLI and GitHub Copilot SDK, so they inherit the same model limitations and safety controls for file access, file modification, command execution, session scoping, and connected tools.
...
* **Exercise caution and evaluate outcomes when using Copilot agentic features for consequential decisions or in sensitive domains**: 
