# Using Copilot CLI as your AI SME
...
When you need to understand an unfamiliar codebase, you usually rely on a teammate who knows the code. When that person isn't available, Copilot CLI can fill the gap. In this tutorial, you'll ask Copilot CLI questions about a repository's code, learn prompting techniques that produce code-grounded answers, and build the confidence to start making changes.
...
The tutorial assumes you're comfortable working in a terminal and reading code in the language used by the project you're exploring. You don't need any prior experience with Copilot CLI.
...
Open a terminal and change into the root of the repository you want to learn about. Starting Copilot CLI from the repository root means it has the whole codebase available as context.
...
You're now in an interactive Copilot CLI session. Anything you type at the prompt is sent to the model, which can read files in the repository, run shell commands such as `git log` or `grep`, and answer your questions based on what it finds.

> [!TIP]
> * If you trust Copilot CLI to run commands without prompting for approval, while blocking file changes, start with `copilot --deny-tool='write' --allow-all-tools`. See [Allowing Tools](https://docs.github.com/en/copilot/how-tos/copilot-cli/use-copilot-cli/allowing-tools).
> * If you're working in a very large codebase, you may get better results by starting Copilot CLI from the relevant subdirectory. This limits the context to the most relevant files.

## 2. Ground answers in the code, not the docs

Most repositories contain a mixture of code and documentation: README files, design notes, architecture diagrams, and so on. Documentation is useful for finding out about a project, but it may be out of date. If you ask Copilot CLI a question about how the code behaves, and it picks up an answer from an out-of-date Markdown file, you'll get a confident answer that is wrong.

Before you start asking questions, tell Copilot CLI to base its answers on the code itself, and to flag any uncertainty rather than fill in gaps. For example:
...
You can put this kind of preamble in a custom instructions file and reuse it across sessions, so you don't have to retype it each time. See [Add Custom Instructions](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-custom-instructions).
...
Asking for citations and evidence is important. When Copilot CLI tells you "this is handled in `src/auth/middleware.ts`," you can open that file and verify the answer for yourself. If the citation doesn't match what you find, that's a useful signal to push back and ask follow-up questions.
...
It's a good idea to ask similar questions that approach a topic from different angles. This helps to ensure you're getting the full picture, and will give you confidence that Copilot hasn't misunderstood what you're trying to find out about, and hasn't missed an important detail.
...
Asking follow-up questions within the same CLI session allows you to drill into the details of a feature and provides Copilot with the context it needs to give you specific answers that tell you what you need to know.

## 5. Investigate behavior and edge cases

Reading code tells you what the code does. It often doesn't tell you what the code is supposed to do, or how it behaves in unusual situations. These are the questions where a human SME is most valuable—and where Copilot CLI can help you when no human is available.
...
A lot of useful context lives in the Git history rather than in the current code: why a function was written the way it was, when a bug was last fixed, who originally added a module. Copilot CLI can run `git` commands for you and use the output to answer questions you couldn't easily answer by reading the code alone.
...
Copilot CLI is a powerful SME, but it's not infallible. It can misread code, conflate similar-looking files, or, occasionally, make things up. Treat its answers the way you'd treat advice from a helpful but fallible coworker: useful as a starting point, but worth checking before you act on anything important.

A few habits will keep you out of trouble:

* **Check the cited files.** When Copilot CLI references a specific file or line number, open it and confirm. If the citation is wrong, treat the rest of the answer with more skepticism.
* **Run things yourself.** If Copilot CLI tells you a function returns `null` for an empty input, write a quick test or REPL invocation that calls it with an empty input and check.
* **Cross-check with humans.** When the answer matters (security, data integrity, anything you'd be embarrassed to get wrong in production), use Copilot CLI's answer as a starting point and confirm with a human SME or a code review.

## Conclusion

In this tutorial, you've used Copilot CLI as your personal SME to:
...
Used this way, Copilot CLI doesn't replace human SMEs, but it makes you less dependent on them for the day-to-day questions that come up while you work. Your human teammates can focus their time on the questions that genuinely need their experience, and you can keep moving when they're not around.
