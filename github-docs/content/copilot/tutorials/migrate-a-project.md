...
* **Use AI to help you**
  ...
  If you don't understand a particular part of the code you are about to translate, use Copilot's "explain" feature, either on the whole file, or a selected portion of the code within a file. See [Chat In Ide](https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide).
...
1. In your editor, open the Copilot Chat panel. See [Chat In Ide](https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide).
1. Ask Copilot to outline the steps you need to take to migrate the project to the new language.
...
1. Copy Copilot's response and save this somewhere to refer to throughout the process.
1. Work through each step of the process, asking for detailed help from Copilot as you need it.
...
1. As soon as you've migrated a component to a state that you can run, check that it works as expected. If it generates an error, copy the error into the Copilot Chat view, and ask Copilot to help you fix it.
1. After you've completed the initial conversion, use Copilot to help you refactor the code in the new language. For more information, see [Refactor Code](https://docs.github.com/en/copilot/tutorials/refactor-code).
...
1. Create a working branch in your repository and open it as a new workspace in Visual Studio Code.
1. Familiarize yourself with the contents and architecture of the existing project. Copilot can help you with this:
...
1. Ask Copilot for a high-level overview of the migration process:
...
1. Copy Copilot's response and save this somewhere to refer to throughout the remainder of this process. You'll work through each of these steps, asking Copilot for help as you proceed.

1. If you haven't already decided which Python framework you want to use, you can ask Copilot for advice:
...
1. Returning to Copilot's high-level overview, the first step is to set up a Python environment and install the necessary packages. To find out how to do this, in Copilot Chat, enter:
...
1. Follow Copilot's instructions and use `pip list` to verify that Flask is installed.
1. Returning to Copilot's overview for the migration process, the next step is to convert the configuration. Ask Copilot for help with this. For example:
...
1. Returning to the overview again, ask Copilot to help you rewrite the PHP functions.
...
1. Continuing with the overview, the next step is to create a Flask application. Ask Copilot to help you with this:
...
1. Copilot may also suggest that, having created these files, you now run `python app.py` in the terminal, from the root of the repository, and then browse to [http://127.0.0.1:5000](http://127.0.0.1:5000).
...
1. Copy the initial `TemplateNotFound` error message to the Copilot Chat view and ask how you can fix this. For example:
