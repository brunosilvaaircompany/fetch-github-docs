# Using deep links to open the GitHub Copilot app
...
You can open the GitHub Copilot app from browser links that wrap an app link in the hosted launcher. The hosted launcher opens the app when possible and shows a fallback page if the browser cannot hand off to the app. For a full list of supported app links, see [Available app links](#available-app-links).
...
* To resume an agent task, use this app link:
   ...
   Use the GitHub task ID for `TASK_ID`. Do not use a runtime session ID or an app-local session URL for links that resume GitHub Copilot agent tasks.
...
| `ghapp://github.com/OWNER/REPO/tasks/TASK_ID` | Agent task | Resumes a GitHub Copilot agent task. Use the GitHub task ID, not an app-local session ID. |
...
| `ghapp://sessions/SESSION_ID` | App session | Opens an app-local workspace or session. Do not use this shape to resume GitHub Copilot agent tasks. |
