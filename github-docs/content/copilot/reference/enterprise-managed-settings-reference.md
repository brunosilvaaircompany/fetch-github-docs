...
| Key | Purpose | Copilot CLI | VS Code | GitHub Copilot app |
| --- | --- | --- | --- | --- |
...
| `telemetry` | Configures OpenTelemetry export, routing Copilot usage data to a collector of your choice | {% octicon "check" aria-label="Supported" %} | {% octicon "check" aria-label="Supported" %} | {% octicon "x" aria-label="Not supported" %} |
...
Restricts whether Copilot CLI sessions hosted on a device can be remotely controlled. This doesn't affect a user's ability to remotely control their sessions hosted on other devices.
