...
| --- | --- |
| `/af` | Finds installable MCP servers, tools, skills, and agents by searching Agent Finder. This slash command is a built-in skill. See [MCP](https://docs.github.com/en/copilot/concepts/context/mcp#agent-finder). |
| `/agent` | Selects a custom agent for the session. See [About Custom Agents](https://docs.github.com/en/copilot/concepts/agents/cloud-agent/about-custom-agents). |
...
| `/autopilot [PROMPT]` | Switches to **Autopilot** mode and optionally starts execution with a prompt. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#choosing-a-session-mode). |
| `/chronicle` | Opens session history and analysis features. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#using-chronicle-with-app-sessions). |
...
| `/interactive [PROMPT]` | Switches to **Interactive** mode and optionally starts with a prompt. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#choosing-a-session-mode). |
...
| `/plan [PROMPT]` | Switches to **Plan** mode and optionally starts with a prompt. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#choosing-a-session-mode). |
...
| `/remote` | **Requires an active session.** Enables or manages remote control so you can access the current session from GitHub.com in a browser or from GitHub Mobile. Availability depends on your organization or enterprise policy. See [About Remote Control](https://docs.github.com/en/copilot/concepts/agents/copilot-cli/about-remote-control). |
...
| `/rubber-duck [PROMPT]` | Requests critique on your approach or implementation by a model other than the one you have used in the session. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#using-the-rubber-duck-agent). |
| `/security-review` | **Requires an active session with changes.** Runs a security-focused review against current diffs. See [Agent Sessions](https://docs.github.com/en/copilot/how-tos/github-copilot-app/agent-sessions#using-security-review-in-app-sessions). |
