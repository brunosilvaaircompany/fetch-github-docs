...
For more information about API versions, see [API Versions](https://docs.github.com/en/rest/about-the-rest-api/api-versions).
...
- **Change `pre_receive_hook_id` path parameter type from integer to string**
  This change ensures that the `pre_receive_hook_id` path parameter is consistently set to type string.
