...
> [!NOTE]
> In REST, the global node ID field is named `node_id`. In GraphQL, it's an `id` field on the `node` interface. For a refresher on what "node" means in GraphQL, see [Introduction To GraphQL](https://docs.github.com/en/graphql/guides/introduction-to-graphql#node).
