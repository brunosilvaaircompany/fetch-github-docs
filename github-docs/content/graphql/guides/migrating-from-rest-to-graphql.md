...
GitHub provides two APIs: a REST API and a GraphQL API. For more information about GitHub's APIs, see [Comparing Githubs Rest API And GraphQL API](https://docs.github.com/en/rest/about-the-rest-api/comparing-githubs-rest-api-and-graphql-api).
...
```shell
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/orgs/:org/members
```
...
```shell
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/pulls
```
...
```shell
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/pulls/:number
```
...
```shell
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/pulls/:number
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/pulls/:number/commits
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/issues/:number/comments
curl -v https://api.github.com{% elsif ghes %}http(s)://HOSTNAME/api/v3/repos/:owner/:repo/pulls/:number/reviews
```
