...
| :- | :- | :- | :- |
| SP Entity ID | SP URL, audience restriction | The top-level URL for your organization on your GitHub Enterprise Server instance | `https://github.com/orgs/ORGANIZATION` |
...
| `SessionNotOnOrAfter` | {% octicon "x" aria-label="Optional" %} | The date that GitHub invalidates the associated session. After invalidation, the person must authenticate once again to access your enterprise's resources. For more information, see [Session duration and timeout](#session-duration-and-timeout). |
|  |
...
| `username` | {% octicon "x" aria-label="Optional" %} | The username for your GitHub Enterprise Server instance. |
|  |
