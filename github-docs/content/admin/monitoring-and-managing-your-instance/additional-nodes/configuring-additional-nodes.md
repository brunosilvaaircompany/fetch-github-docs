...
> [!NOTE]
> Support for additional nodes in a high availability configuration is currently in public preview and subject to change.
...
If an additional node is offline, unreachable, or on a different version, or if `ghe-remove-node` or a verification check fails, contact GitHub Support. Do not edit `cluster.conf` manually.
