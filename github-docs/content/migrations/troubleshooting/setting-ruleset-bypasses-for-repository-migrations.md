# Setting ruleset bypasses for repository migrations

## Background

When you import a repository with GitHub Enterprise Importer, an internal service pushes the repository's Git data to GitHub, batching refs together for performance. This introduces a possible case where evaluation of organization-level or enterprise-level rulesets against the pushed refs can time out, causing the repository migration as a whole to fail.

Because GitHub Enterprise Importer tags its Git pushes as migration operations, you can create ruleset bypasses that apply only to repository migrations and not to other actors. This prevents ruleset evaluations from timing out and causing migration failures.
...
As noted above, GitHub Enterprise Importer uses a specific path for pushing Git data, and the **Repository migrations** bypass only targets that path—there is no path where ordinary users or Git pushes to existing repositories can bypass rulesets using the **Repository migrations** bypass.
...
* [Troubleshooting Your Migration With GitHub Enterprise Importer](https://docs.github.com/en/migrations/troubleshooting/troubleshooting-your-migration-with-github-enterprise-importer)
* [About Rulesets](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/about-rulesets)
* [Managing Policies For Code Governance](https://docs.github.com/en/admin/enforcing-policies/enforcing-policies-for-your-enterprise/managing-policies-for-code-governance)
